"""
main.py — Entry point. APScheduler BackgroundScheduler.
...
def job_scrape():
    ...
    try:
        ...
        fb_new = scraper.run_fb_scrape()
        total  = kj_new + cl_new + eb_new + fb_new
        db.log_agent_event("scrape",
            f"Done: {total} total new (KJ:{kj_new} CL:{cl_new} EB:{eb_new} FB:{fb_new})")
        db.finish_job_run(run_id, "done", f"new:{total}")
        if total > 0:
            send_alert_telegram(f"🔍 Scraped {total} new listings (KJ:{kj_new} CL:{cl_new} EB:{eb_new} FB:{fb_new})")
        ...
        for l in listings:
            try:
                ...
                if score >= 70:
                    send_alert_telegram(
                        f"🏅 High-score deal!\n{l['title'][:60]}\n"
                        f"Score: {score}/100 | ${l.get('price_cad',0):.0f} CAD\n{l.get('url','')}")
                ok += 1
            except Exception as e:
            ...
            for listing in queued:
                ...
                try:
                    if platform == "facebook":
                        ok, conv_url = await messenger.send_message_facebook(
                            listing["url"], msg_text, fb_ctx, p)
                    elif platform == "kijiji":
                        ok, conv_url = await messenger.send_message_kijiji(
                            listing["url"], msg_text, kj_ctx, p)
                    else:
                        db.log_agent_event("message",
                            f"Skipping {platform} — messaging not automated")
                        db.update_listing_status(listing["id"], "scored")
                        continue

                    if ok:
                        db.save_conversation(listing["id"], platform, conv_url,
                            [{"role":"me","text":msg_text,"ts":"now"}])
                        ...
                        failed += 1
                        db.log_agent_event("message",
                            f"Failed to send to '{listing['title'][:40]}'", level="warn")

                    await safety.inter_message_delay(platform)
                except Exception as e:
                    db.log_agent_event("message",
                        f"Error for listing {listing['id']}: {e}", level="error")
                    failed += 1
        ...
        async with async_playwright() as p:
            ...
            for listing in active:
                try:
                    ...
                    platform = listing.get("platform","")
                    if platform == "facebook":
                        messages = await poller.read_fb_conversation(
                            conv["conversation_url"], fb_ctx)
                    elif platform == "kijiji":
                        messages = await poller.read_kijiji_conversation(
                            conv["conversation_url"], kj_ctx)
                    else:
                        continue
                    ...
                    if should_ho:
                        ...
                        db.log_agent_event("poll",
                            f"🏆 HANDOVER: '{listing['title'][:40]}' score {final_score}")
                        continue
                    ...
                    if parsed.get("seller_replied") and follow_up_count < 2:
                        ...
                        if followup:
                            if platform == "facebook":
                                ok,_ = await messenger.send_message_facebook(
                                    conv["conversation_url"], followup, fb_ctx, p)
                            else:
                                ok,_ = await messenger.send_message_kijiji(
                                    conv["conversation_url"], followup, kj_ctx, p)
                            if ok:
                                db.increment_followup_count(listing["id"])
                                db.update_listing_status(listing["id"], "replied")
                                db.log_agent_event("poll",
                                    f"Follow-up #{follow_up_count+1} sent to '{listing['title'][:35]}'")
...
def job_daily_summary():
    stats = {
        "found":          db.get_count("today"),
        "high_score":     db.get_count("score_gte_70"),
        "messages_sent":  db.messages_sent_today(),
        "awaiting_reply": db.get_count("awaiting_reply"),
        "handovers":      db.get_count("handover"),
        "ghosted":        db.get_count("ghosted"),
    }
    send_daily_summary_telegram(stats)
    db.log_agent_event("daily", f"Summary sent: {stats}")

# ── SCHEDULE ──────────────────────────────────────────────────────────────────
scheduler.add_job(job_scrape,        IntervalTrigger(minutes=20), id="scrape")
scheduler.add_job(job_score,         IntervalTrigger(minutes=25), id="score")
scheduler.add_job(job_message,       IntervalTrigger(minutes=30), id="message")
scheduler.add_job(job_poll,          IntervalTrigger(minutes=45), id="poll")
scheduler.add_job(job_daily_summary, 'cron', hour=20, minute=0,   id="daily_summary")

# ── TEST MODE ─────────────────────────────────────────────────────────────────
def run_test_mode():
    print("\n" + "="*60 + "\n  TEST MODE — One Full Cycle\n" + "="*60)
    print("\n[1/4] SCRAPE"); job_scrape()
    print("\n[2/4] SCORE");  job_score()
    print("\n[3/4] MESSAGE"); job_message()
    print("\n[4/4] POLL");   job_poll()
    print("\n" + "="*60)
    print("  DONE — open dashboard: streamlit run dashboard/app.py")
    print("="*60)
